...
const STATUS_LABEL = {
  ...
  completed: { label: 'Дууссан',        color: 'bg-gray-100 text-gray-500'    },
};
...
export default function MyBookings() {
  ...
  const BookingCard = ({ b }) => {
    ...
    return (
      <div className="border border-gray-200 rounded-2xl p-4 bg-white hover:shadow-sm transition-all">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-sm text-dark leading-snug">{b.service_name}</div>
            <div className="text-xs text-gray-500 mt-0.5">👩‍🦱 {b.artist_name}</div>
          </div>
  ...
  return (
    <div className="overlay active" onClick={e => { if (e.target === e.currentTarget) closeMyBookings(); }}>
      <div className="modal modal-sheet bg-[#FFFAF5] rounded-[28px] w-full max-w-[560px] max-h-[88vh] overflow-y-auto p-8 relative border border-gold/15 shadow-[0_32px_80px_rgba(0,0,0,.18)] max-[640px]:p-5" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="font-serif text-[22px] font-semibold text-dark mb-1 max-[640px]:text-lg">Миний захиалгууд</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <button onClick={() => { closeMyBookings(); openBooking(); }}
              className="bg-gradient-to-r from-[#B8960C] via-[#D4AF37] to-[#C9A84C] text-white border-none px-6 py-2.5 rounded-full text-sm font-semibold cursor-pointer">
              Цаг захиалах
            </button>
          </div>
        ) : (
          <>
            {upcoming.length > 0 && (
              <div className="mb-6">
                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Удахгүй болох ({upcoming.length})</div>
            ...
            {past.length > 0 && (
              <div>
                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Өнгөрсөн / Цуцлагдсан ({past.length})</div>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              <button onClick={() => { closeMyBookings(); openBooking(); }}
                className="w-full bg-gradient-to-r from-[#B8960C] via-[#D4AF37] to-[#C9A84C] text-white border-none py-3 rounded-full text-sm font-semibold cursor-pointer">
                + Шинэ захиалга
              </button>
            </div>
          </>

...
export default function Trainings() {
  ...
  return (
    <section id="trainings" className="py-[70px] px-12 bg-[#FFFAF5] max-[900px]:px-5 max-[900px]:py-12 max-[640px]:px-4 max-[640px]:py-9">
      {/* Header */}
            ...
            return (
              <div key={t.id} onClick={() => setSelected(t)}
                ...
                <div className="px-3 py-3 flex-1 flex flex-col">
                  <div className="text-[13px] font-semibold text-dark mb-1 leading-[1.3]">{t.title}</div>
                  {t.description && (
                  ...
                  {(t.duration || t.schedule) && (
                    <div className="text-[10px] text-gray-400 mb-2.5 truncate">
                      {[t.duration && `⏱ ${t.duration}`, t.schedule && `📅 ${t.schedule}`].filter(Boolean).join(' · ')}
                    </div>
                  )}
                  <div className="flex items-center justify-between gap-2 mt-auto">
                    {t.price > 0
                      ? <span className="font-display text-[15px] font-bold text-gold-dark max-[640px]:text-sm">₮{t.price.toLocaleString()}</span>
                      : <span className="text-[11px] text-gray-400">Үнэ тохиролцоно</span>}
      ...
      {selected && (
        <div className="overlay active" onClick={e => { if (e.target === e.currentTarget) setSelected(null); }}>
          <div className="modal modal-sheet bg-[#FFFAF5] rounded-[28px] w-full max-w-[640px] max-h-[90vh] overflow-y-auto relative border border-gold/15 shadow-[0_32px_80px_rgba(0,0,0,.18)]" onClick={e => e.stopPropagation()}>
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-[#B8960C] via-[#D4AF37] to-[#C9A84C] rounded-t-[28px]" />
            <button onClick={() => setSelected(null)}
              className="absolute top-5 right-5 w-9 h-9 rounded-full border border-gold/20 bg-gold/8 cursor-pointer text-base text-dark/40 flex items-center justify-center hover:bg-gold/20 z-10">
              ✕
            </button>
            ...
            <div className="p-8 max-[640px]:p-5">
              {selected.level && (
                <span className="inline-block bg-gold/10 text-gold-dark border border-gold/20 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide mb-3">
                  {selected.level}
                </span>
              )}
              <h2 className="font-serif text-[26px] font-semibold text-dark mb-4 max-[640px]:text-[22px]">{selected.title}</h2>
                ...
                {[
                  ...
                  <div key={lbl} className="flex items-center gap-3 bg-gold-light/40 border border-gold/15 rounded-xl px-3 py-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[13px] font-semibold text-dark">{val}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {selected.description && (
                <div className="text-[14px] text-gray-600 leading-[1.85] mb-6 whitespace-pre-wrap">{selected.description}</div>
              )}

              <a href="#contact" onClick={() => setSelected(null)}
                className="btn-shine block w-full bg-gradient-to-r from-[#B8960C] via-[#D4AF37] to-[#C9A84C] text-white text-center border-none py-3.5 rounded-full text-[14px] font-bold cursor-pointer transition-all shadow-[0_4px_18px_rgba(201,168,76,.40)] hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(201,168,76,.55)] no-underline">

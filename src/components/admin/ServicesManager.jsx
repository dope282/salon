...
const EMPTY = {
  name_mn: '', description_mn: '', price_from: '', duration_min: '60', emoji: '✂️', active: true,
};
...
export default function ServicesManager({ showToast }) {
  ...
  const openEdit = (s) => { setForm({ ...s, price_from: String(s.price_from), duration_min: String(s.duration_min) }); scrollToForm(); };
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {services.map(s => (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 14px', borderRadius: 14, border: '1.5px solid var(--gray-200)', background: s.active ? '#fff' : 'var(--gray-100)', opacity: s.active ? 1 : .7, transition: 'all .2s', flexWrap: 'wrap' }}>
                ...
                {/* Emoji icon */}
                <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg,#FFD6E8,#FFBCD9)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
                  {s.emoji}
                </div>
      ...
      {form && (
        <div className="card" style={{ border: '2px solid var(--pink-light)' }}>
          ...
          <div style={{ borderTop: '1.5px solid var(--gray-200)', paddingTop: 16, marginBottom: 20 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--gray-500)', display: 'block', marginBottom: 10, textTransform: 'uppercase', letterSpacing: .8 }}>Emoji дүрс</label>
